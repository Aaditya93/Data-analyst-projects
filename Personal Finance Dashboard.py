...
# requried for manipulating data
import pandas as pd
import numpy as np
# requried for building inactive dashboard
import panel as pn
...
df['Description'] = df['Description'].map(str.lower) # lower case for descriptions
df = df.rename(columns ={'Completed Date':'Date'})
df['Category'] = 'unasigned'
df.head()
...
# Assign transactions to correct category

# self-care
df['Category'] =np.where(df['Description'].str.contains('cash at tesco old st h exp|boots|royal'),'Self-Care',df['Category'])
...
# Check unassigned transactions and confirm all the transactions are assigned to a cateogry 
...
# Create the top banner for summary of last month's income, recurring expenses , non-recurring expenses , saving

# get lastest month and year
lastest_month = df['Month'].max()
lastest_year = df['Year'].max()
 
# filter the dataframe to include only transactions from last month 
last_month_expenses = df[(df['Month'] == lastest_month) & (df['Year'] == lastest_year) ]
...
#exclude "excluded" category
last_month_expenses = last_month_expenses[last_month_expenses["Category"].str.contains('Excludes | unassigned') == False]
last_month_expenses = last_month_expenses.sort_values(by = 'Amount', ascending = False) # sort values
last_month_expenses = last_month_expenses['Amount'].round().astype(int) # round values
last_month_expenses
